...
def configure_cors_middleware(app):
    ...
    default_origins = ["http://localhost:5173", "http://127.0.0.1:5173"]  # Vite dev server

    # Check for CORS_DOMAINS environment variable first
    cors_domains_env = os.environ.get('CORS_DOMAINS')
    if cors_domains_env:
        cors_options["allow_origins"] = [origin.strip() for origin in cors_domains_env.split(',') if origin.strip()]
